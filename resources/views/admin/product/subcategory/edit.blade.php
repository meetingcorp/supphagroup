...
@php $pagename = 'แก้ไขหมวดหมู่ย่อย' @endphp
                    ...
                    <div class="card-header">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <form method="post" action="{{route('sub.update', ['sub' => $sub->id])}}" enctype="multipart/form-data">
                                    @method('PUT')
                                    @csrf
                                    <label for="title">ชื่อหมวดหมู่ย่อย</label><br/>
                                    <input name="title" type="text" class="form-control" required value="{{$sub->title}}"><br/>
                                    ...
                                    <select name="maincategory" id="maincategory" class="form-control" required>
                                        @foreach ($maincategory as $item) 
                                            <option value="{{$item->id}}" @if($item->id == $sub->categories_id) selected @endif>{{$item->title}}</option>
                                        @endforeach
                                    </select>
